...
def run(
    ...
    # 修改结果保存路径
    save_dir = increment_path(Path('runs/phone') / name, exist_ok=exist_ok)
    # 创建路径文件
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if names[int(cls)] == "cell phone":
                            label = None if hide_labels else (names[c] if hide_conf else f'{"phone"} {conf:.2f}')
                            annotator.box_label(xyxy, label, color=colors(c, True))
                            #save_crop = True
                            if save_crop:
                                save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
                        if names[int(cls)] == "person":
                            label = None if hide_labels else (names[c] if hide_conf else f'{"people"} {conf:.2f}')
                            annotator.box_label(xyxy, label, color=colors(c, True))
                            #save_crop = True
                            if save_crop:
                                save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
